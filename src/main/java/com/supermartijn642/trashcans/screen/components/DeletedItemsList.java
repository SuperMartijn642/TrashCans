...
public class DeletedItemsList extends BaseWidget {
    ...
    public static final ResourceLocation TOGGLE_TAB = TrashCans.identifier("toggle_tab");
    ...
    private class SlotsBar extends BaseWidget {
        ...
        public void renderBackground(WidgetRenderContext context, GuiGraphicsHelper graphics, int mouseX, int mouseY){
            ...
            float x = this.x();
            graphics.submitDefaultScreenBackground(x, this.y, this.width, this.height);
            for(int i = 0; i < slots; i++)
                graphics.submitDefaultSlot(x + 4, this.y + 4 + i * 18);
        }
            ...
            if(DeletedItemsList.this.expansionProgress != 0){
                ...
                for(int i = 0; i < slotCount; i++){
                    ...
                    if(!slot.isActive()){
                        ...
                        if(!stack.isEmpty())
                            graphics.submitItem(stack, x + 5, this.y + 5 + i * 18);
                    }
                }
            }
        }
    ...
    private class Toggle extends AbstractButtonWidget {
        ...
        public void renderBackground(WidgetRenderContext context, GuiGraphicsHelper graphics, int mouseX, int mouseY){
            super.renderBackground(context, graphics, mouseX, mouseY);
            if(this.isClickable())
                graphics.submitSprite(TOGGLE_TAB, this.x(), this.y, this.width, this.height, p -> p.uv(expanded ? 0.5f : 0, this.isFocused() ? 1 / 3f : 0, 0.5f, 1 / 3f));
        }
    }
}
