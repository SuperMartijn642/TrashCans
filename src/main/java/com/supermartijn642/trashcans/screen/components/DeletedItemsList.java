...
public class DeletedItemsList extends BaseWidget {
    ...
    private final Supplier<ContainerScreen<?>> screen;
    private SlotsBar slotsBar;
    private Toggle toggle;
    private float expansionProgress = expanded ? 1 : 0;

    public DeletedItemsList(int x, int y, List<CustomSlot> slots, Supplier<List<ItemStack>> items, Supplier<ContainerScreen<?>> screen){
    ...
    private class SlotsBar extends BaseWidget {
        ...
        public void renderBackground(MatrixStack poseStack, int mouseX, int mouseY){
            ...
            float x = this.x();
            ScreenUtils.drawScreenBackground(poseStack, x, this.y, this.width, this.height);
            ScreenUtils.bindTexture(SLOT_TEXTURE);
            for(int i = 0; i < slots; i++)
                ScreenUtils.drawTexture(poseStack, x + 4, this.y + 4 + i * 18, 18, 18);
        }
        ...
        public void render(MatrixStack poseStack, int mouseX, int mouseY){
            ...
            if(DeletedItemsList.this.expansionProgress != 0){
                ...
                GlStateManager._translatef(x, 0, 0);
                int slotCount = DeletedItemsList.this.slotCount();
                for(int i = 0; i < slotCount; i++){
                    ...
                    if(!slot.isActive()){
                        ...
                        if(!stack.isEmpty())
                            ClientUtils.getItemRenderer().renderGuiItem(stack, 5, this.y + 5 + i * 18);
                    }
                }
    ...
    private class Toggle extends AbstractButtonWidget {
        ...
        public void renderBackground(MatrixStack poseStack, int mouseX, int mouseY){
            ...
            if(this.isClickable()){
                ...
                ScreenUtils.drawTexture(poseStack, 0, this.y, this.width, this.height, expanded ? 0.5f : 0, this.isFocused() ? 1 / 3f : 0, 0.5f, 1 / 3f);
                poseStack.popPose();
            }
        }
    }
