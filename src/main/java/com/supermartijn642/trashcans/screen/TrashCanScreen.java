...
public abstract class TrashCanScreen<T extends TrashCanContainer> extends ObjectBaseContainerWidget<TrashCanBlockEntity,T> {
    ...
    @Override
    protected void renderBackground(PoseStack poseStack, int mouseX, int mouseY, TrashCanBlockEntity entity){
    ...
    @Override
    protected void renderForeground(PoseStack poseStack, int mouseX, int mouseY, TrashCanBlockEntity entity){
        super.renderForeground(poseStack, mouseX, mouseY, entity);
        ScreenUtils.drawCenteredString(poseStack, this.title, this.width() / 2f, 6);
        ScreenUtils.drawString(poseStack, ClientUtils.getPlayer().getInventory().getName(), 21, this.height() - 94);
