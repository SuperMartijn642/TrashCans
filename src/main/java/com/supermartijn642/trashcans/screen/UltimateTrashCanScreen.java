...
public class UltimateTrashCanScreen extends TrashCanScreen<UltimateTrashCanContainer> {
    ...
    protected void renderTooltips(int mouseX, int mouseY, TrashCanBlockEntity entity){
        ...
        if(this.leftArrow.isFocused() && this.leftArrow.isActive())
            ScreenUtils.drawTooltip("" + (this.shift ? this.control ? -100000 : -100 : this.control ? -10000 : -1000), mouseX, mouseY);
    ...
    protected void drawText(TrashCanBlockEntity entity){
        ScreenUtils.drawString(TextComponents.translation("trashcans.gui.ultimate_trash_can.item_filter").get(), 8, 53);

        ScreenUtils.drawString(TextComponents.translation("trashcans.gui.ultimate_trash_can.liquid_filter").get(), 8, 83);

        ScreenUtils.drawString(TextComponents.translation("trashcans.gui.ultimate_trash_can.energy_limit").get(), 8, 113);
        ScreenUtils.drawCenteredString(TextComponents.string(I18n.format("trashcans.gui.energy_trash_can.value").replace("$number$", "" + entity.getEnergyLimit())).get(), 114, 132);
    }
